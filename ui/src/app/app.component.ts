    ...
    <ion-app>
      ...
      <ion-content class="ion-padding">
        ...
        <ng-template #detail>
          ...
          <ng-template #empty><p>Select an assistant from the list.</p></ng-template>
        </ng-template>
      </ion-content>
    </ion-app>
  `,
...
export class AppComponent {
  readonly title = "AI Part Lookup Assistant";
  readonly screens = ["Assistants", "Assistant Detail"];
  readonly screen = signal("Assistants");
  readonly items = signal<Assistant[]>([]);
  readonly selected = signal<Assistant | null>(null);
  readonly error = signal('');
  private readonly http = inject(HttpClient);

  constructor() {
    this.http.get<Assistant[]>(apiUrl('/api/assistants')).subscribe({
      next: (items) => this.items.set(items),
      error: () => this.error.set('Could not reach the API.'),
    });
  }

  select(item: Assistant): void {
